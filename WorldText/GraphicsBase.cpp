...
//    std::cout << path_to_ttf_ << endl;
//    cout << font_loading_size_ << endl;
...
Text::Text()
{
    Text(Constants::DEFAULT_FONT_PATH_PREFIX+Constants::DEFAULT_TTF_FILE, Constants::DEFAULT_FONT_LOAD_SIZE);
}
...
void Text::RenderText(std::string text, GLfloat x, GLfloat y, GLfloat scale)
{
    ...
    glUseProgram(text_program_id);
//    glUniform3f(glGetUniformLocation(text_program_id, "textColor"), color.x, color.y, color.z);
...
TextUtil::TextUtil(const Window & window_)
        :TextUtil(window_, Constants::DEFAULT_FONT_PATH_PREFIX+Constants::DEFAULT_TTF_FILE, Constants::DEFAULT_FONT_LOAD_SIZE)
{
    ;
}
TextUtil::TextUtil(const Window & window_, const std::string & path_to_ttf_)
        :TextUtil(window_, path_to_ttf_, Constants::DEFAULT_FONT_LOAD_SIZE)
{

}
TextUtil::TextUtil(const Window & window_, const std::string & path_to_ttf_, unsigned int font_load_size_)
        :window(window_)
{
...
void TextUtil::RenderText(const std::string & text, const glm::vec2 & point_, const glm::vec3 & color_, GLfloat scale_)
{
    auto x = point_.x;
    auto y = point_.y;
